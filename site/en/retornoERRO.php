...
if(isset($_GET['check'])){
	switch($_GET['check']){
		case 'true':
			$say = "Your order has been processed. Shortly you will receive an email with your download instructions. Order number $pedido.";
			$isOk = true;
			break;
		case 'false':
			$say = 'Your order has not been processed.';
			break;
		case 'cancel':
			$say = 'Your order has been cancelled.';
			break;
		case 'check':
			$say = 'Your order has been processeda.';
			break;
	}
}
...
if($isOk) {
	...
	if($lingua!="br") {
		$fullcc .= ", saulo@pulsarimages.com, icaro@pulsarimages.com";
	}
