...
//	define('SIGNATURE_PAYPAL','AFcWxV21C7fd0v3bYYYRCpSSRl31AwRR42uND6pSn5Y6fPKcGS-rA1VX');
	...
	foreach($_SESSION['produto'] as $id => $quantidade){
		...
		//print_r($produto);
		$idFoto = $produto[0]['Id_Foto'];
		$idproduto = $produto[0]['tombo'];
		$nomeproduto = $produto[0]['tombo']." | ".$produto[0]['assunto_principal'];
		$assunto = $produto[0]['assunto_principal'];
		$uso = $_SESSION['produto'.$id]['uso'];
		$vlunitario = $_SESSION['produto'.$id]['valor'];
		$vlunitario = ($lingua!="br"?convertPounds($vlunitario):$vlunitario);
		$subtotal =  $vlunitario;
		$totalgeral += $subtotal;
		...
		$nvp['L_PAYMENTREQUEST_0_NAME'.$arrayPayPal] = CARRINHO_TIPO_PRODUTO;
		$nvp['L_PAYMENTREQUEST_0_DESC'.$arrayPayPal] = $idproduto;
		...
		<tbody>
			<tr>
			  <td>
				<div class="imagem">
				  <img src="http://www.pulsarimagens.com.br/bancoImagens/'.$id.'p.jpg" width="120" />
				</div>
				<div class="descricao">
				  <ul>
					<li class="autor"><span class="label">Author:</span> '.$produto[0]['Nome_Fotografo'].'</li>
					<li class="codigo"><span class="label">Code:</span> '.$id.'</li>
<!--					<li class="uso"><span class="label">Descrição:</span> '.$assunto.'</li>-->
				  </ul>
				</div>
			  </td>
			  <td class="acoes">
				<div class="calculo-preco">
				  <div class="preco">GBP '.number_format($vlunitario,2,'.',',').'</div>
				</div>
			  </td>
			</tr>
		 </tbody>
		...
		<tfoot>
			<tr>
			  <td colspan="2" class="total">Total: GBP '.number_format($totalgeral,2,'.',',').'</td>
			</tr>
		</tfoot>
		...
		if(isset($_GET['tipopag'])){
			...
			$dt = date("d/m/Y H:i:s");
			$pedido->gravaPedido(array("cpf" => $row_top_login['id_cadastro'], "cod_pedido" => $_SESSION['existPedido'], "valorpedido" => $totalgeral, "moeda" => "GPB", "data" => $dt,"statustransacao" => "Aguardando pagamento"));	
			/////////////////
		}

/*
/// configurar dados do cliente para pagseguro
$objetopagseguro->setSender($row_top_login['nome'], $row_top_login['email']);
...
$nvp['PAYMENTREQUEST_0_CURRENCYCODE']='GBP';
...
$nvp['LOCALECODE']='en_UK';
$nvp['RETURNURL']='http://www.pulsarimagens.com.br/en/ecommerce/retornoPaypal.php';
$nvp['CANCELURL']='http://www.pulsarimagens.com.br/en/retornoERRO.php?check=cancel';
