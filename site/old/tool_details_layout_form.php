<?php
if($row_top_login['download']!='S' && $row_top_login['download']!='L') {
...
//echo "Add arquivo $source_file<br>";
...
//echo "Arquivo de destino $dest_file<br>";
...
//echo "Colocando IPTC $dest_file<br>";
 ...
 */	
		...
		if(count($files_arr) > 1) {
			foreach ($files_arr as $file_erase) {
				if(siteDebug) {
//					echo "<br>Criando zip com os seguintes arquivos: $file_erase.<br>";
				}
			}

			$zipfile = $homeftp."/temp/".$usuario.".zip";
	//		$zipfile = 'c:\\'.$usuario.".zip";
				if(!create_zip($files_arr,$zipfile) && $siteDebug) {
					echo "<br>Erro criando zip $zipfile.<br>";
				}
