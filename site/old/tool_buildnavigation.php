...
function buildNavigation($actual, $maxRows_retorno, $totalRows_query, $display_qty = 3, $separator = " ... ") {
    ...
    if($pages_qty > 1) {
	    if($actual > 0) {
	    	$precedente = $actual - 1;
	    	$firstArray = "<a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$precedente$_get_vars\" class=\"ant\">« Anteriores</a>";
			$debug .= "First! ";
	    }
	    else {
	    ...
	    if ($actual < $pages_qty-1) {
	    	$successivo = $actual + 1;
	    	$lastArray = "<a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$successivo$_get_vars\" class=\"pro\">Próximas »</a>";
			$debug .= "Last! ";
	    }
	    else {
	    ...
	    for($a = 0; $a < $pages_qty; $a++) {
	    	...
	    	if($a == 0) {
	    		if($a ==  $actual)
					$pagesArray .= "<div class=\"pag\"><a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$a$_get_vars\"><strong>1</strong></a>";
				else 
					$pagesArray .= "<div class=\"pag\"><a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$a$_get_vars\">1</a>";
	    	}
	    	else if(($a < $display_qty + 2) && $actual < $display_qty) {
				$page = $a + 1;
	    		if($a ==  $actual)
					$pagesArray .= "<a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$a$_get_vars\"><strong>$page</strong></a>";
	    		else 
					$pagesArray .= "<a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$a$_get_vars\">$page</a>";
			}
