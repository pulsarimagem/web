...
// Sem group by
	$query_arquivos = sprintf("SELECT * FROM log_download2,cadastro WHERE date_format(data_hora, '%%m/%%Y') = '%s' and cadastro.id_cadastro = log_download2.id_login GROUP BY arquivo,projeto,EXTRACT(DAY FROM data_hora) order by usuario", $colname_arquivos);
} else {
...
// Sem group by
	$query_arquivos = sprintf("SELECT * FROM log_download2 WHERE id_login = '%s' GROUP BY arquivo,projeto,EXTRACT(DAY FROM data_hora) order by data_hora desc", $colname_arquivos);
}
