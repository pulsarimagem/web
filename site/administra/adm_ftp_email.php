...
<table width="700" border="1" cellpadding="3" cellspacing="0" bordercolor="#666666">
  <tr>
    <td class="style4">Cliente para o  envio: </td>
    <td><select name="to">
      <?php
do {  
?>
      <option value="<?php echo $row_ftps['email']?>"><?php echo $row_ftps['nome']?></option>
      <?php
} while ($row_ftps = mysql_fetch_assoc($ftps));
  $rows = mysql_num_rows($ftps);
  if($rows > 0) {
      mysql_data_seek($ftps, 0);
	  $row_ftps = mysql_fetch_assoc($ftps);
  }
?>
    </select>
