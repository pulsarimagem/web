...
//	$query_arquivos = sprintf("SELECT * FROM log_download2 WHERE date_format(data_hora, '%%m/%%Y') = '%s' GROUP BY arquivo, EXTRACT(DAY FROM data_hora) order by data_hora desc", $colname_arquivos);
	$query_arquivos = sprintf("SELECT * FROM log_download_layout,cadastro WHERE date_format(data_hora, '%%m/%%Y') = '%s' and cadastro.id_cadastro = log_download_layout.id_login GROUP BY arquivo, EXTRACT(DAY FROM data_hora) order by usuario", $colname_arquivos);
} else {
	$query_arquivos = sprintf("SELECT * FROM log_download_layout WHERE id_login = '%s' GROUP BY arquivo, EXTRACT(DAY FROM data_hora) order by data_hora desc", $colname_arquivos);
}
...
$query_periodo = "SELECT date_format(data_hora,'%m/%Y') as mes_ano FROM log_download_layout group by mes_ano order by data_hora desc";
...
FROM
  cadastro
  INNER JOIN log_download_layout ON (cadastro.id_cadastro = log_download_layout.id_login)
WHERE
...
mysql_select_db($database_pulsar, $pulsar);
$query_diretorios = "SELECT * FROM cadastro WHERE download = 'S' OR download = 'L' ORDER BY cadastro.nome";
...
do {  
?>
    <option value="<?php echo $row_diretorios['id_cadastro']?>"<?php if (!(strcmp($row_diretorios['id_cadastro'], $_POST['id_login']))) {$limite = $row_diretorios['limite']; echo "selected=\"selected\"";} ?>><?php echo $row_diretorios['nome']?> / <?php echo $row_diretorios['empresa']?></option>
    <?php
...
do {  
?>
    <option value="<?php echo $row_periodo['mes_ano']; ?>"<?php if (!(strcmp($row_periodo['mes_ano'], $_POST['periodo']))) {echo "selected=\"selected\"";} ?>><?php echo $row_periodo['mes_ano']?></option>
    <?php
...
<table width="750" border="0">
  <tr class="style5">
    ...
    <td bgcolor="#CCCCCC"><div align="center">Layouts</div></td>
...
<? } while ($row_tabela1 = mysql_fetch_assoc($tabela1));  ?>
  <tr class="style5">
...
<? } while ($row_tabela2 = mysql_fetch_assoc($tabela2));  ?>
  <tr class="style5">
    <td colspan="2" bgcolor="#CCCCCC">Total:</td>
    <td bgcolor="#CCCCCC"><div align="center"></div></td>
    <td bgcolor="#CCCCCC"><div align="center"></div></td>
...
if ($totalRows_arquivos > 0) { // Show if recordset not empty ?>
    <span class="style5">Layouts retirados:</span><br>
    <br>
    <table width="550" border="0" cellspacing="0" cellpadding="2">
	  ...
	  $contador = 0;
	   ...
	   if ($mes != makeDateTime($row_arquivos['data_hora'], 'm/Y')) {
		  ...
		  ?>
        <tr>
            <td colspan="4" bgcolor="#CCCCCC" class="style5"><div align="left"><a href="#" onClick="MM_openBrWindow('adm_layout_misc.php?id_login=<?php echo $colname_arquivos ;?>&data=<?php echo $mes; ?>','','')">Total de downloads do m&ecirc;s <?php echo $mes; ?>: <?php echo $contador;?></a></div></td>
         </tr>
	   <?php
...
<?php } else { ?>
	<span class="style7">Sem layouts retirados!!! </span><?php }; ?>
	<br>
	<br>
<?php }; ?>
    ...
    <table width="700" border="0" cellspacing="0" cellpadding="2">
	  ...
	  $contador = 0;
	   ...
	   if ($mes != makeDateTime($row_arquivos['data_hora'], 'm/Y')) {
		  ...
		  ?>
        <tr>
            <td colspan="4" bgcolor="#CCCCCC" class="style5"><div align="left"><a href="#" onClick="MM_openBrWindow('adm_layout_misc.php?id_login=<?php echo $colname_arquivos ;?>&data=<?php echo $mes; ?>','','')">Total de downloads de layouts do m&ecirc;s <?php echo $mes; ?>: <?php echo $contador;?></a></div></td>
         </tr>
	   <?php
