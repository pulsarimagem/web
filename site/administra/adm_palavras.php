...
if (isset($_GET['busca_fracao'])) {
	$colname_palavra_chave = (get_magic_quotes_gpc()) ? $_GET['busca_fracao'] : addslashes($_GET['busca_fracao']);
...
if (isset($_GET['todas_total'])) {
	$colname_palavra_chave = (get_magic_quotes_gpc()) ? $_GET['busca'] : addslashes($_GET['busca']);
	$query_palavra_chave = sprintf("SELECT pal_chave.Id, pal_chave.Pal_Chave, count(rel_fotos_pal_ch.id_rel) AS total FROM pal_chave INNER JOIN rel_fotos_pal_ch ON (pal_chave.Id=rel_fotos_pal_ch.id_palavra_chave) GROUP BY pal_chave.Id, pal_chave.Pal_Chave ORDER BY pal_chave.Pal_Chave");
	echo $query_palavra_chave;
	$palavra_chave = mysql_query($query_palavra_chave, $pulsar) or die(mysql_error());
	$row_palavra_chave = mysql_fetch_assoc($palavra_chave);
}

if (isset($_GET['inicial']) || isset($_GET['busca']) || isset($_GET['busca_fracao'])) {
	mysql_select_db($database_pulsar, $pulsar);
...
<form name="formBusca" action="adm_palavras.php" method="get" >
Busca Fração: <input name="busca_fracao" type="text" id="busca">
</form>
<br>
<br>
<?php if (isset($_GET['inicial']) || isset($_GET['busca']) || isset($_GET['busca_fracao']) || isset($_GET['todas']) || isset($_GET['todas_total'])) {?>
...
<?php if(isset($_GET['todas'])) { ?>  
    <td class="style4"><a href="../br/listing.php?so_pc=<?php echo $row_palavra_chave['Pal_Chave']; ?>&pc_action=Ir&type=pc&tipo=inc_pc.php&pc_action=Ir" target="_blank"><?php echo $row_palavra_chave['Pal_Chave']; ?></a></td>
<?php } else if(isset($_GET['todas_total'])) { ?>  
    <td class="style4"><?php echo $row_palavra_chave['Pal_Chave']; ?> (<?php echo $row_palavra_chave['total'];?>)</td>
<?php } else { ?>
    <td class="style4"><a href="../br/listing.php?so_pc=<?php echo $row_palavra_chave['Pal_Chave']; ?>&pc_action=Ir&type=pc&tipo=inc_pc.php&pc_action=Ir" target="_blank"><?php echo $row_palavra_chave['Pal_Chave']; ?> (<?php echo $row_palavra_chave['total'];?>)</a>
