...
//  echo $insertSQLp;
  ...
  $insertGoTo = "adm_index_video.php";
//  if (isset($_SERVER['QUERY_STRING'])) {
...
// DELETAR REGISTRO
if ((isset($_POST["MM_delete"])) && ($_POST["MM_delete"] == "form4")) {
	$deleteSQL = sprintf("DELETE FROM Fotos_tmp WHERE Id_Foto=%s",
			GetSQLValueString($_POST['del_cromo'], "int"));
//echo 	$deleteSQL;
	mysql_select_db($database_pulsar, $pulsar);
...
//echo 	$deleteSQL;
	...
	$updateSQL = sprintf("UPDATE codigo_foto SET status=-2 WHERE codigo=%s",
			GetSQLValueString($_POST['tombo'], "text"));
	//echo 	$updateSQL;
	mysql_select_db($database_pulsar, $pulsar);
	$Result1 = mysql_query($deleteSQL, $pulsar) or die(mysql_error());
...
*/
mysql_select_db($database_pulsar, $pulsar);
$query_autor_tmp_select = sprintf("SELECT Fotos_tmp.id_autor, count(Fotos_tmp.id_autor) as total,fotografos.Nome_Fotografo as nome FROM Fotos_tmp, fotografos WHERE Fotos_tmp.tombo RLIKE '^[a-zA-Z]' AND Fotos_tmp.id_autor = fotografos.id_fotografo AND Fotos_tmp.status = 2 GROUP BY id_autor");
...
if(isset($id_autor)) {
	mysql_select_db($database_pulsar, $pulsar);
	$query_fotos_tmp_select = sprintf("SELECT tombo FROM Fotos_tmp WHERE Fotos_tmp.tombo RLIKE '^[a-zA-Z]' AND Fotos_tmp.status = 2 AND id_autor=%s LIMIT 20",$id_autor);
...
<form action="adm_index_video.php" method="get" name="form3" class="style2">
   Lista Autores: 
   <select name="autor2">
...
<?php if($id_autor) { ?>
<form action="adm_index_video.php" method="get" name="form1" class="style2">
   Lista Tombo: 
   <select name="tombo2">
   		<option value="nada">--- Nenhuma ---</option>
...
<?php if($totalRows_dados_foto > 0) {
	$data = file_get_contents($cloud_server.'get_thumbs.php?&tombo='.$_GET['tombo']);
	$thumbs = json_decode($data);

	$data = file_get_contents($cloud_server.'get_video_id3.php?tombo='.$_GET['tombo']);
...
?>
          ...
          <tr>
			...
			<td class="style2"><input name="pal_chave" type="text" id="pal_chave" value="<?php echo $pal_chave_txt; ?>" size="70"><input name="Button" type="button" onClick="MM_openBrWindow('adm_index_desc2.php?Id_Foto=<?php echo $id_foto; ?>&tombo=<?php echo $tombo; ?>&Pal_Chave=<?php echo $row_dados_foto['pal_chave'];?>','','scrollbars=yes,width=470,height=280,top=500,left=250')" value="Incluir Estes Descritores"></td>
		  </tr>
          <tr>
...
?>
          ...
          <tr>
            <td colspan="2" class="style6"><div align="center"><br>
                ...
                <input name="del_cromo" type="hidden" value="<?php echo $id_foto?>">
...
<form name="form4" method="POST">
  	<input type="hidden" name="MM_delete" value="form4"/>
	<input type="button" name="Button" value="Excluir" onClick="excluir();"/>
