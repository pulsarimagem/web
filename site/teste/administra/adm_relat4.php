...
$query_pcs = sprintf("SELECT 
  ...
  count( log_pop.tombo ) as total
FROM
 log_pop
 INNER JOIN Fotos ON (log_pop.tombo=Fotos.tombo)
WHERE
  log_pop.datahora >= %s AND
  log_pop.datahora <= %s
GROUP BY
  log_pop.tombo
ORDER BY
  total DESC
LIMIT 200", GetSQLValueString($colname_pcs1, "date"), GetSQLValueString($colname_pcs2, "date"));
...
<p><strong>Top 200 Pop's </strong></p>
<table width="720" border="1">
  <tr>
    <td>Tombo - Assunto Principal</td>
    <td><div align="center">Total de Pops</div></td>
  </tr>
  <?php do { ?>
    <tr>
      <td><?php echo $row_pcs['tombo'].' - '.$row_pcs['assunto_principal']; ?></td>
