...
if($row_dados_foto['dim_a']!=NULL &&$row_dados_foto['dim_a']!=0) {
	if($isVideo) {
		$detail_dim .= "<p class=\"label\">Dimensões do vídeo:</p> ".$row_dados_foto['dim_a']." x ".$row_dados_foto['dim_b']."<br/><br/>";
	}else {
		$detail_dim .= "<p class=\"label\">Dimensões da imagem:</p> ".$row_dados_foto['dim_a']." px X ".$row_dados_foto['dim_b']." px   -   300 DPI. Caso necessite esse arquivo em outras dimensões, <a href=\"contato.php\">entre em contato.</a><br/><br/>";
	}
}
