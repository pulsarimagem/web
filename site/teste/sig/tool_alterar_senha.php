...
$query_cadastro = sprintf("SELECT * FROM USUARIOS WHERE usuario='%s'", $row_top_login['usuario']);
$cadastro = mysql_query($query_cadastro, $sig) or die(mysql_error());
$row_cadastro = mysql_fetch_assoc($cadastro);
$totalRows_cadastro = mysql_num_rows($cadastro);

$login = $row_cadastro['usuario'];
...
if(!$has_error && $submit) {
	...
	if ($loginFoundUser) {
		...
		if ($loginFoundPass) {
			...
			$updateSQL = sprintf("UPDATE USUARIOS SET senha=%s WHERE usuario=%s",
                       GetSQLValueString($_POST['nova_senha'], "text"),
//                       GetSQLValueString($_POST['email'], "text"),
					   GetSQLValueString($loginUsername, "text"));

			$Result1 = mysql_query($updateSQL, $sig) or die(mysql_error());
