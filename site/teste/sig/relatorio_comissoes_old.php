...
$total_indio = 0;
$comissoes_indios = array("Kalapalo" => 0, "Caigangue" => 0, "Bororo" => 0, "Yanomami" => 0, "Kamayurá" => 0, "Xavante" => 0, "Yawalapiti" => 0, "Guarani" => 0);
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<?php
				...
				while ($row_objRsContratos = mysql_fetch_assoc($objRsContratos)) {
					...
					If($row_objRsContratos["ID_CONTRATO_DESC"] == "5" || $row_objRsContratos["ID_CONTRATO_DESC"] == "9")
						$indio = true;
					...
					if($indio) {
						$autor_comissao = (($row_objRsContratos["VALOR_TOTAL"] - $impvalor) - $cliente_comissao) / 3;
							...
							IF ($row_OBJCONTRATOS["ID_CONTATO"] != "") {
								...
								if($row_OBJCONTATO = mysql_fetch_assoc($OBJCONTATO)) {				
									...
									if($row_OBJCONTATO["COMISSAO"] != "") {
										...
										$COMISSAO_AUTOR     = (($CROMO_VALOR - $IMPOSTO_VALOR) - $COMPRADOR_COMISSAO) / $DIVISOR_AUTOR;
									}
									else {
									...
									$COMISSAO_AUTOR = ($CROMO_VALOR - $IMPOSTO_VALOR) / $DIVISOR_AUTOR;		
								}
							}								
							ELSE {
								$COMISSAO_AUTOR = ($CROMO_VALOR - $IMPOSTO_VALOR) / $DIVISOR_AUTOR;
...
<?php if(!$print) { ?>		
		<tr><td colspan="3">&nbsp;&nbsp;&nbsp;<a href="<?php echo $_SESSION['menu_url'] ?>"><b>Clique aqui</b></a> para realizar nova consulta.</td></tr>
<?php } ?>		
		</tbody>
	</table>
<?php if(!$print) { ?>
...
<!--                  <td>COMPRADOR</td>-->	
			...
			While($row_objRsContratos = mysql_fetch_assoc($objRsContratos)) {
				...
				If(isIndio($row_objRsContratos["ID_CONTRATO_DESC"])) {
					$indio = true;
				}
