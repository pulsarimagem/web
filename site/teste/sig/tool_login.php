...
//	mysql_select_db($database_pulsar, $pulsar);
    ...
    strtoupper(get_magic_quotes_gpc() ? $loginUsername : addslashes($loginUsername))); 
echo $LoginRS__query;
  	
	$LoginRS = mysql_query($LoginRS__query, $sig) or die(mysql_error());
	...
	if ($loginFoundUser) {
    	...
    	strtoupper(get_magic_quotes_gpc() ? $loginUsername : addslashes($loginUsername)), strtoupper(get_magic_quotes_gpc() ? $password : addslashes($password)));
echo $LoginRS__query;
...
//		$loginFoundPass = ($senha==$loginUsername."2012");
		...
		if ($loginFoundPass) {
			...
			$_SESSION['MM_Username_Sig'] = $loginUsername;
			$_SESSION['MM_UserGroup_Sig'] = $loginStrGroup;
