...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1") && ($_POST["Id1"] > 0)) {
  $insertSQL = sprintf("INSERT INTO rel_fotos_temas_tmp (id_foto, id_tema) VALUES (%s, %s)",
...
$query_incluir = sprintf("SELECT 
  rel_fotos_temas_tmp.id_rel,
  rel_fotos_temas_tmp.id_foto,
  rel_fotos_temas_tmp.id_tema,
  super_temas.Tema_total
FROM
 rel_fotos_temas_tmp
 INNER JOIN super_temas ON (rel_fotos_temas_tmp.id_tema=super_temas.Id)
WHERE (rel_fotos_temas_tmp.id_foto = %s AND rel_fotos_temas_tmp.id_tema = %s)
...
WHERE (rel_fotos_temas_tmp.id_foto = %s AND rel_fotos_t_tmpemas.id_tema = %s)
 ORDER BY id_rel DESC
 limit 1",
                        GetSQLValueString($_POST['id_foto'], "int"),
                       GetSQLValueString($_POST['Id3'], "int"));
