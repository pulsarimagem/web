...
if(isset($_GET['chkbox'])) {
	$sql = "SELECT tombo FROM pasta_fotos WHERE id_foto_pasta IN ('".implode("','",$_GET['chkbox'])."')";
	$arrTombos = array();
	$result = mysql_query($sql, $pulsar) or die(mysql_error());
	while($row = mysql_fetch_assoc($result)) {
		$arrTombos[] = $row['tombo'];
	}
	$_GET['tombo'] = $arrTombos[0];
}
if(isset($_GET['id_pasta'])) {
	$url_ok = "emailsucesso.php?anterior=primeirapagina.php?id_pasta=".$_GET['id_pasta'];
	$url_cancel = "primeirapagina.php?id_pasta=".$_GET['id_pasta'];
}
else { 
	$url_ok = "emailsucesso.php?anterior=details.php?tombo=".$_GET['tombo']."%26search=PA";
	$url_cancel = "details.php?tombo=".$_GET['tombo']."&search=PA";
}
...
if (isset($_GET['action'])) {
	...
	if(($nome = $_GET['nome']) == "") {
		$nome_error = true;
		$nome_error_msg = ENVIAIMAGEM_NOME_ERROR;
		$has_error = true;
	}
	if(($remetente = $_GET['rementente']) == "") {
		$remetente_error = true;
		$remetente_error_msg = ENVIAIMAGEM_EMAIL_ERROR;
		$has_error = true;
	}
	if(($email = $_GET['email']) == "") {
		$email_error = true;
		$email_error_msg = ENVIAIMAGEM_EMAIL_ERROR;
		$has_error = true;
	}
	if(($mensagem = $_GET['mensagem']) == "") {
		$mensagem_error = true;
		$mensagem_error_msg = ENVIAIMAGEM_MENSAGEM_ERROR;
		$has_error = true;
	}
	if(($assunto = $_GET['assunto']) == "") {
		$assunto_error = true;
		$assunto_error_msg = ENVIAIMAGEM_ASSUNTO_ERROR;
		$has_error = true;
	}
}
if (isset($_GET['tombo'])||isset($arrTombos)) {
	mysql_select_db($database_pulsar, $pulsar);
	if(is_array($arrTombos))
		$tombos = implode("','",$arrTombos);
	else 
		$tombos = $_GET['tombo'];
