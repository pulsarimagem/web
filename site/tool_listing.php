...
if(isset($_GET['ajustar']) && $_GET['ajustar'] == "ajustar") { // logica inversa
...
*/	
	...
	if (isset($_GET['tombo'])&& ($_GET['tombo']!= "")) {
		...
		if (isset($_GET['tema']) && $_GET['tema'] != "") {
			...
			if($lingua != "br")
				$linguaTema = "_en";
			$sql = "SELECT Tema$linguaTema AS tema FROM temas WHERE Id = $idTema";
	...
	//else if ((isset($_GET['pc_action']))||(isset($_GET['pa_action']))) {
		...
		if(isset($_GET['so_pc']) && $_GET['so_pc']!="") {
			$pesq = new elementoPesquisa();
			$pesq->arrPalavras[$_GET['so_pc']] = $lingua;
			$pesq->arrCampos['pc'] = true;
			$pesq->pc = 1;
			$pesquisas[] = $pesq;
			$engine->isEnable = true;
