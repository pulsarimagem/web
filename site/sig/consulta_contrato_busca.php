...
$razao = removeAccent($razao);
$fantasia = removeAccent($fantasia);
...
//realizando a consulta por nome fantasia e/ou período
	...
	if($razao != "") {
		if($add_where)
			$filter_razao = " AND ";
		$filter_razao .= " convert(RAZAO using utf8) LIKE _utf8 '%$razao%'";
		$add_where = true;
	}
	if($fantasia != "") {
		if($add_where)
			$filter_fantasia = " AND ";
		$filter_fantasia .= " convert(FANTASIA using utf8) LIKE _utf8 '%$fantasia%'";
		$add_where = true;
	}
