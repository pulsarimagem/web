...
//			alert(val);
			$.ajax("tool_ajax.php?chkIndio="+thisCheck.attr('value')).done(function (){
				atualiza_valor_cromo();
			});
		}
		else {
...
//			alert(val);
//			alert("tool_ajax.php?reuso="+thisCheck.attr('value')+"&false=false");
			$.ajax("tool_ajax.php?chkIndio="+thisCheck.attr('value')+"&false=false").done(function (){
				atualiza_valor_cromo();
			});
//			location.reload();
		}
...
function mascara_ie() { //Abre a função
	var c = document.form.inscricao.value.length; /*Cria a variavel que verifica a quantidade de caractes digitados no primeiro formulariio da pagina que tenha um campo que se chama isdsc */
	if(c == 3){ /* Verifica se o que foi digitado no campo é igual a 3, se for igual.. */
		document.form.inscricao.value += "."; /*Adiciona na frente do que estiver ja digitado um ponto */
	} //Fecha a verificação
