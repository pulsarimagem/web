	...
	if( $qcodigo_arr == "" ) {
		...
		foreach($qcodigo_arr as $qcodigo) {
			...
			if(!$row_rs2 = mysql_fetch_assoc($rs2)) {
				$mens="<< Cromo não existente. Verifique o código digitado. >>";
...
echo $strSQL;
	...
	header("location: consulta_contrato_visualiza.php?id_contrato=$id_contrato&mens=$mens");//?mens=Contrato nº".id_contrato." registrado com sucesso.");
}
else {
	header("location: consulta_contrato_visualiza.php?editaCromo=sim&id_contrato=$id_contrato&mens=$mens");
}
if($_POST['cromo_nao_cadastrado'] == "s") {
	header("location: consulta_contrato_visualiza.php?editaCromo=sim&cromo_nao_cadastrado=s&id_contrato=$id_contrato&mens=$mens");
}
else if($_POST['exec'] == "finaliza") {
	header("location: consulta_contrato_visualiza.php?id_contrato=$id_contrato&mens=$mens");//?mens=Contrato nº".id_contrato." registrado com sucesso.");	
}
