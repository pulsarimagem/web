...
<title>MOVIMENTO/CONTRATO/CONSULTA .::SIG - SISTEMA DE INFORMAÇÕES
...
<body>
	<form action="consulta_contrato_baixa.php" method="post">
		<?php
		//resgatando parâmetros enviados pelo form via método get
		$id_contrato = isset($_GET['id_contrato'])?$_GET['id_contrato']:(isset($_POST['id_contrato'])?$_POST['id_contrato']:"");
		$fantasia 	= isset($_GET['fantasia'])?$_GET['fantasia']:(isset($_POST['fantasia'])?$_POST['fantasia']:"");
		$de			= isset($_GET['de'])?$_GET['de']:(isset($_POST['de'])?$_POST['de']:"");
		$ate		= isset($_GET['ate'])?$_GET['ate']:(isset($_POST['ate'])?$_POST['ate']:"");
		$id_cliente	= isset($_GET['id_cliente'])?$_GET['id_cliente']:(isset($_POST['id_cliente'])?$_POST['id_cliente']:"");
		$baixa	= isset($_GET['baixa'])?true:(isset($_POST['baixa'])?true:false);
		$lote	= isset($_GET['lote'])?true:(isset($_POST['lote'])?true:false);
		...
		if ($action == "Concluir") {
			//requisitando parâmetros
			$id_contratos		= isset($_GET['id_contrato'])?$_GET['id_contrato']:$_POST['id_contrato'];
			//	id_contrato_S 	= Split(id_contrato,",")
			//	id_contrato_U 	= UBound(id_contrato_S)
			$mens		 	= "";
			if($lote) {
				$baixa_lote	= isset($_GET['baixa_lote'])?$_GET['baixa_lote']:$_POST['baixa_lote'];
				$nf_lote	= isset($_GET['nf_lote'])?$_GET['nf_lote']:$_POST['nf_lote'];
			}

			foreach ($id_contratos as $id_contrato) {
				if($lote) {
					$baixa	= isset($_GET['baixa'.Trim($id_contrato)])?"true":(isset($_POST['baixa'.Trim($id_contrato)])?"true":"");
					If ($baixa != "") {
						$nf = $nf_lote;
						$baixa = $baixa_lote;
					}
				} else {
					$baixa	= isset($_GET['baixa'.Trim($id_contrato)])?$_GET['baixa'.Trim($id_contrato)]:$_POST['baixa'.Trim($id_contrato)];
					$nf		= isset($_GET['nf'.Trim($id_contrato)])?$_GET['nf'.Trim($id_contrato)]:$_POST['nf'.Trim($id_contrato)];
				}
			...
			if($mens == "") {
				echo "Campos vazios. Nenhuma baixa efetuada.<br /><a href=\"javascript:history.back();\"><b>Voltar para a página anterior</b></a>";
			} else {
				?>
		Baixa(s) efetuada(s) com sucesso.<br /> <br /> Relação de contratos
		baixados para simples conferência:<br />
		<table>
			<tr>
				<td id="border"><center>
		...
		//realizando a consulta por id do contrato
			...
			$strSQL    = "SELECT CONTRATOS.ID, CONTRATOS.ID_CLIENTE, CONTRATOS.DATA, CONTRATOS.DESCRICAO, CONTRATOS.DATA_PAGTO, CONTRATOS.NOTA_FISCAL, sum(cast(replace(replace(VALOR,'.',''),',','.') as decimal(10,2))-cast(replace(replace(DESCONTO,'.',''),',','.') as decimal(10,2))) AS VALOR_TOTAL FROM CROMOS LEFT JOIN CONTRATOS ON CONTRATOS.ID = CROMOS.ID_CONTRATO WHERE CONTRATOS.ID='".$id_contrato."' AND CONTRATOS.FINALIZADO = 'S' AND BAIXADO='N' GROUP BY CONTRATOS.ID ";
			$objRS = mysql_query($strSQL, $sig) or die(mysql_error());
		...
		<table width="95%">
			<tr>
				...
				<td id="border" width="10%"><center>
						<font face="Times New Roman" color="#000000"><b>EMISSÃO</b> </font>
					</center></td>
				<td id="border" width="10%"><center>
						<font face="Times New Roman" color="#000000"><b>CLIENTE</b> </font>
					</center></td>
				<td id="border" width="%"><center>
						<font face="Times New Roman" color="#000000"><b>DESCRIÇÃO</b> </font>
					</center></td>
				<td id="border" width="10%"><center>
		...
		<table width="95%">
			...
			<tr>
				...
				<td id="border" width="50%"><center>
						<font face="Times New Roman" color="#000000"><b>RAZÃO SOCIAL</b> </font>
					</center></td>
		...
		<?php
			...
			$strSQL	= "SELECT CONTRATOS.ID, CONTRATOS.DATA, CONTRATOS.DESCRICAO, CONTRATOS.DATA_PAGTO, CONTRATOS.NOTA_FISCAL, SUM(cast(replace(replace(CROMOS.VALOR,'.',''),',','.') as decimal(10,2))-cast(replace(replace(CROMOS.DESCONTO,'.',''),',','.') as decimal(10,2))) AS VALOR_TOTAL
			FROM CROMOS
		...
		<table>
			...
			<tr>
				...
				<td id="border">de: <b><?php echo $de?> </b>&nbsp;&nbsp;&nbsp;até: <b><?php echo $ate?>
				</b>
				</td>
			</tr>
		...
		<table width="95%">
			...
			<tr>
				<td id="border"><p align="right">Razão Social:
