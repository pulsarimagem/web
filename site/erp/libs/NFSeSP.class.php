...
class NFSeSP
{
  private $cnpjPrestador = '66647157000111'; // Your CNPJ

  private $ccmPrestador = '98980084'; // Your CCM

  private $passphrase = 'associacao'; // Cert passphrase

  private $certDir = '/var/www/www2.pulsarimagens.com.br/site/erp/certs'; // Dir for .pem certs

  private $pkcs12  = "/var/www/www2.pulsarimagens.com.br/site/erp/pulsar.pfx";
  
//   private $pkcs12  = 'C:/certs/certificado_teste.pfx';
//   private $passphrase = 'foitec'; // Cert passphrase
  
//   private $pkcs12  = 'C:/certs/foiteste.pfx';
  ...
  public function __construct()
  {
    ...
    if ( $this->loadCert() ) {
      ...
      printf( __METHOD__ . ': Certificado nao OK!' );
    }
  }
  ...
  private function validateCert( $cert )
  {
    ...
    if ( $certValidDate < time() ){
      printf( __METHOD__ . ': Certificado expirado em ' . date( 'Y-m-d', $certValidDate ) );
      return false;
    }
  ...
  private function loadCert()
  {
    ...
    if ( ! openssl_pkcs12_read( file_get_contents( $this->pkcs12 ), $x509CertData, $this->passphrase ) ) {
      printf( __METHOD__ . ': Certificado nao pode ser lido. O arquivo esta corrompido ou em formato invalido.' );
    ...
    if ( ! is_dir( $this->certDir ) ) {
      if ( ! mkdir( $this->certDir, 0777 ) ) {
        printf( __METHOD__ . ': Falha ao criar o diretorio ' . $this->certDir );
        return false;
      }
    }
    ...
    if ( ! file_exists( $this->privateKey ) ) {
      if ( ! file_put_contents( $this->privateKey, $x509CertData['pkey'] ) ) {
        printf( __METHOD__ . ': Falha ao criar o arquivo ' . $this->privateKey );
        return false;
      }
    }
    ...
    if ( ! file_exists( $this->publicKey ) ) {
      if ( ! file_put_contents( $this->publicKey, $x509CertData['cert'] ) ) {
        printf( __METHOD__ . ': Falha ao criar o arquivo ' . $this->publicKey );
        return false;
      }
    }
    ...
    if ( ! file_exists( $this->key ) ) {
      if ( ! file_put_contents( $this->key, $x509CertData['cert'] . $x509CertData['pkey'] ) ) {
        printf( __METHOD__ . ': Falha ao criar o arquivo ' . $this->key );
        return false;
      }
    }
  ...
  public function start()
  {
    ...
    try {
      ...
      echo "erro de conexao soap. Tente novamente mais tarde !<br>\n";
      echo $e->getMessage();
      return false;
    }
  }
...
//    exit();
    ...
    printf( __METHOD__ . ': ' . $xmlDoc->saveXML() );
...
//      openssl_sign( $digestValue, $signatureValue, $pkeyId );
  ...
  public function textFile( $rangeDate, $valorTotal, $rps )
  {
    ...
    if ( ! file_put_contents( $rpsFullPath, $header . $body . $footer ) ) {
      printf( __METHOD__ . ': Cannot create rps file ' . $rpsFullPath );
      return false;
    }
