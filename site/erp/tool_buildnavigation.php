...
function buildNavigation($actual, $maxRows_retorno, $totalRows_query, $display_qty = 3, $separator = " ... ") {
    ...
    if($pages_qty > 1) {
	    if($actual > 0) {
	    	$precedente = $actual - 1;
	    	$firstArray = "<a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$precedente$_get_vars\" class=\"ant\">« Anterior</a>";
			$debug .= "First! ";
	    }
	    else {
	    ...
	    if ($actual < $pages_qty-1) {
	    	$successivo = $actual + 1;
	    	$lastArray = "<a href=\"$_SERVER[PHP_SELF]?pageNum_retorno=$successivo$_get_vars\" class=\"pro\">Próxima »</a>";
			$debug .= "Last! ";
	    }
	    else {
