...
$query_fotos_download = sprintf("SELECT downloads.usuario, downloads.id_login, downloads.tombo, downloads.data_hora, downloads.circulacao, downloads.tiragem, downloads.projeto, downloads.formato, downloads.uso, downloads.obs, downloads.ip, fotografos.Nome_Fotografo, fotografos.Iniciais_Fotografo, Fotos.data_foto, Fotos.cidade, Estados.Estado, Estados.Sigla, Fotos.assunto_principal, paises.nome as pais, sig.USO_DESC.descricao as tamanho_desc, sig.USO_SUBTIPO.subtipo as uso_desc 
									from Fotos 
									...
									LEFT JOIN sig.USO_SUBTIPO ON (downloads.uso = sig.USO_SUBTIPO.Id)
									LEFT JOIN sig.USO ON (downloads.formato = sig.USO.Id)
									LEFT JOIN sig.USO_DESC ON (sig.USO.id_descricao = sig.USO_DESC.Id);", $row_top_login['id_cadastro'], $mes, $ano);
