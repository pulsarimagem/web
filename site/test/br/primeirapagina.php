...
<form name="form1" method="post" action="primeirapagina.php?id_pasta=<?php echo $_GET['id_pasta']?>">
	<div class="adicionarimagem" style="margin: -225px 0 0 -200px;">
	    ...
	    <h2>Copiar para Outras Pastas</h2>
	    <div class="imagens">
...
<?php } while ($row_pastas = mysql_fetch_assoc($pastas));?>
	    ...
	    <p class="button"><input name="action" type="submit" class="button" value="Copiar" /></p>
	</div>
</form>
<?php } ?>
...
<?php if($show_welcome) {?>
			<h2 class="titulo-pagina">Bem vindo, <strong><?php echo $nome; ?></strong></h2>
			<p class="p">Essa é a sua página dentro do nosso site. Aqui você pode armazenar suas imagens prediletas e utilizar nossas ferramentas para organizá-las em pastas, enviá-las por email, realizar cotações especifícas para suas necessidades e fazer o download das imagens.</p>
<?php } ?>
