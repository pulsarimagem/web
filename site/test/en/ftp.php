...
mysql_select_db($database_pulsar, $pulsar);
$query_pastas = sprintf("SELECT ftp.id_ftp,   cadastro.login,   ftp.id_login,   ftp_arquivos.nome, ftp_arquivos.flag,  ftp_arquivos.data_cria,addDate(ftp_arquivos.data_cria,   ftp_arquivos.validade) as validade,   ftp_arquivos.observacoes,   ftp_arquivos.tamanho, ftp_arquivos.id_arquivo FROM cadastro  INNER JOIN ftp ON (cadastro.id_cadastro=ftp.id_login)  INNER JOIN ftp_arquivos ON (ftp_arquivos.id_ftp=ftp.id_login) WHERE ((cadastro.login LIKE '%s') AND (( TO_DAYS(ftp_arquivos.data_cria) - TO_DAYS(NOW()) + ftp_arquivos.validade )  >=0 )) AND ftp_arquivos.flag ".($show_videos?" RLIKE 'V'":" IS NULL")." ORDER BY ftp_arquivos.nome ", $colname_pastas);
...
<?php if($totalRows_pastas == 0) {?>				
					<div class="error-msg">
						You have no available media at the moement.
					</div>
<?php } else {?>				
					<div class="minhasimages-pastas">
<?php if ($show_videos) { ?>
					<a href="ftp.php">Photos</a></br>
<?php } else { ?>
					<a href="ftp.php?videos=true">Videos</a></br>
					<input name="action" type="image" src="./images/bt-download.png" class="button" value="Baixar todos os arquivos" />
<?php } ?>
						<br/>
						<br/>
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<thead>
								<tr>
									...
									<td>Available until <span>&nbsp;</span></td>
									<td>Comments <span>&nbsp;</span></td>
								</tr>
							</thead>
							<tbody>
<?php do {?>
...
<?php } ?>
									<td class="form">
<?php if($show_videos) { ?>									
										<a href="<?php echo $cloud_server?>downloader_videos.php?u=<?php echo $row_top_login['login']?>&p=<?php echo $row_top_login['senha']?>&c=<?php echo $row_pastas['nome']; ?>&s=<?php echo strstr($row_pastas['flag'],'H')?"hd":"sd"?>" class="link"><?php echo $row_pastas['nome']; ?></a>
<?php } else { ?>
