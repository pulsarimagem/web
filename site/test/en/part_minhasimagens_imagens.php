...
<?php if ($totalRows_fotos_pasta == 0) { ?>				
					<div class="error-msg">
						Your folder is empty. Would like <a href="buscaavancada.php">to search for some media?</a>
					</div>
<?php } else { ?>
					...
					<div class="palheta-top">
            			<div class="top-pasta block">
							My folders - <?php echo $row_nome_pasta['nome_pasta'];?>
						</div>
						<div class="clear"></div>
          			</div>
...
do { 
	...
	mysql_select_db($database_pulsar, $pulsar);
	$query_palavras = sprintf("SELECT  DISTINCT  pal_chave.Pal_Chave$idioma as Pal_Chave,   pal_chave.Id FROM  rel_fotos_pal_ch  INNER JOIN Fotos ON (rel_fotos_pal_ch.id_foto=Fotos.Id_Foto)  INNER JOIN pal_chave ON (pal_chave.Id=rel_fotos_pal_ch.id_palavra_chave) WHERE   (Fotos.tombo LIKE '%s' AND Pal_Chave$idioma IS NOT NULL) group by Pal_Chave$idioma order by Pal_Chave$idioma", $row_fotos_pasta['tombo']);
										...
										<div class="descricao">
											<ul>
						                      ...
						                      <!--<li class="assunto"><span class="label">Assunto:</span> <?php echo $row_fotos_pasta['assunto_principal']; ?></li>-->
												...
												<?php if ($row_fotos_pasta['pais']<>"") { ?> <li class="pais"><span class="label">Country: </span><?php echo $row_fotos_pasta['pais']; ?></li><?php } ?>
...
<?php } while ($row_temas = mysql_fetch_assoc($temas)); ?>
												...
												<li class="descritores"><span class="label">Keywords:</span><br/> |
<?php do { ?>
													<a href="listing.php?query=<?php echo $row_palavras['Pal_Chave']; ?>&pc_action=Ir&type=pc&tipo=inc_pc.php&clear=true"><?php echo $row_palavras['Pal_Chave']; ?></a> | 
